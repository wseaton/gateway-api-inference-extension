...
// Package dispatch provides the factory and registration mechanism for all `framework.InterPriorityDispatchPolicy`
// implementations.
...
// PolicyConstructor defines the function signature for creating a `framework.InterPriorityDispatchPolicy`.
type PolicyConstructor func() (framework.InterPriorityDispatchPolicy, error)
...
func MustRegisterPolicy(name RegisteredPolicyName, constructor PolicyConstructor) {
	...
	if _, ok := RegisteredPolicies[name]; ok {
		panic(fmt.Sprintf("InterPriorityDispatchPolicy already registered with name %q", name))
	}
	RegisteredPolicies[name] = constructor
}

// NewPolicyFromName creates a new `InterPriorityDispatchPolicy` given its registered name.
// This is called by the `registry.FlowRegistry` when configuring the dispatch policy.
func NewPolicyFromName(name RegisteredPolicyName) (framework.InterPriorityDispatchPolicy, error) {
	mu.RLock()
	defer mu.RUnlock()
	constructor, ok := RegisteredPolicies[name]
	if !ok {
		return nil, fmt.Errorf("no InterPriorityDispatchPolicy registered with name %q", name)
	}
	return constructor()
}
